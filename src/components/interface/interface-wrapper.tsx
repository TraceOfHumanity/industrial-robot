...
const InterfaceWrapper = ({ children }: InterfaceWrapperProps) => {
    ...
    return (
        <div className={cn("rounded-lg backdrop-blur-sm bg-white/10 p-2 h-[calc(100vh-1rem)] absolute top-2 right-2 border-l border-white/40", isOpen ? "w-1/4 h-[calc(100vh-1rem)]" : "w-fit h-fit")}>
            {children}
        </div>
    )
}
